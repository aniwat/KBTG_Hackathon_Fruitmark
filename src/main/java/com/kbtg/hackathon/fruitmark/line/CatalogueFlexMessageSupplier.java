...
public class CatalogueFlexMessageSupplier implements Supplier<FlexMessage> {
	...
	public FlexMessage get() {
		final Bubble bubble1 = createBubble("Arm Chair, White", "49.99", "https://2553d2b9.ngrok.io/img/bubble1.png", false);
		final Bubble bubble2 = createBubble("Metal Desk Lamp", "11.99", "https://2553d2b9.ngrok.io/img/bubble2.png", true);
	...
	private Bubble createSeeMoreBubble() {
		return Bubble.builder().body(Box.builder().layout(FlexLayout.VERTICAL).spacing(FlexMarginSize.SM)
		    .contents(asList(Button.builder().flex(1).gravity(FlexGravity.CENTER).action(new URIAction("See more", "http://www.amazon.com")).build())).build()).build();
	}
	...
	private Box createBodyBlock(String title, String price, Boolean isOutOfStock) {
		...
		final Box priceBlock = Box.builder().layout(FlexLayout.BASELINE)
		    ...
		    .build();
		final Text outOfStock = Text.builder().text("Temporarily out of stock").wrap(true).size(FlexFontSize.XXS).margin(FlexMarginSize.MD).color("#FF5551").build();
	...
	private Box createFooterBlock(Boolean isOutOfStock) {
		...
		final Button addToCartDisableButton = Button.builder().style(Button.ButtonStyle.PRIMARY).color("#aaaaaa").action(new URIAction("Add to Cart", "http://example.com")).build();
		final Button addToWishlistButton = Button.builder().action(new URIAction("Add to wishlist", "http://example.com")).build();
		return Box.builder().layout(FlexLayout.VERTICAL).spacing(FlexMarginSize.SM)
		    .contents(asList((!isOutOfStock) ? addToCartEnableButton : addToCartDisableButton, addToWishlistButton)).build();
	}
